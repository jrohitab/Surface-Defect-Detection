...
vgg_model = build_model(VGG19, input_shape=image_size + (3,), num_classes=num_classes)
...
print("\nTraining VGG19...")
vgg_model.fit(train_data, epochs=epochs, validation_data=test_data, verbose=2)

# === EVALUATE ===
xception_acc = xception_model.evaluate(test_data, verbose=0)[1]
vgg_acc = vgg_model.evaluate(test_data, verbose=0)[1]

# === RESULTS ===
print("\n🔍 Accuracy Comparison on Custom Dataset:")
print(f"✅ Xception Accuracy: {xception_acc * 100:.2f}%")
print(f"✅ VGG19 Accuracy:    {vgg_acc * 100:.2f}%")

# === PLOT ===
models = ['Xception', 'VGG19']
accuracies = [xception_acc * 100, vgg_acc * 100]

plt.bar(models, accuracies, color=['skyblue', 'pink'])
plt.ylabel('Accuracy (%)')
plt.title('Xception vs VGG19 on Custom Dataset')
plt.ylim(0, 100)
plt.show()
